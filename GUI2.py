...
class shoppingcart:
    ...
    def computeprice(self):
        ...
        item = 'Product Summary: ' + self.tkvar.get() +','+' '+'Quantity: ' + self.quantityVar.get() + ','+' '+ 'Color: '+ self.tkvar3.get()
        self.summaryVar.set(item)
    
    def getSubtotal(self, tkvar, quanity):
        item_name = self.tkvar.get()
        if item_name == 'Tent':
            subtotal_price = (quanity * 100) - (quanity * 100 * 0.02)
        if item_name == 'Hammocks':
            subtotal_price = (quanity * 120) - (quanity * 120 * 0.02)
        if item_name == 'Gas Stove':
            subtotal_price = (quanity * 50) - (quanity * 50 * 0.02)
        else:
            subtotal_price = (quanity * 40) - (quanity * 40 * 0.02)
        return round(subtotal_price,2)
    def getTaxPrice(self, tkvar, quanity):
        item_name = self.tkvar.get()
        if item_name == 'Tent':
            tax_price = quanity * 100 * 0.02 * 0.13
        if item_name == 'Hammocks':
            tax_price = quanity * 120 * 0.02 * 0.13
        if item_name == 'Gas Stove':
            tax_price = quanity * 50 * 0.02 * 0.13
        else:
            tax_price = quanity * 40 * 0.02 * 0.13
        return round(tax_price,2)

    def getdiscountprice(self, tkvar, quanity):
        item_name = self.tkvar.get()
        if item_name == 'Tent':
            discount_price = quanity * 100 * 0.02
        if item_name == 'Hammocks':
            discount_price = quanity * 120 * 0.02
        if item_name == 'Gas Stove':
            discount_price = quanity * 50 * 0.02
        else:
            discount_price = quanity * 40 * 0.02
        return round(discount_price,2)
    def getTotalprice(self, tkvar, quanity):
    
        item_name = self.tkvar.get()
        if item_name == 'Tent':
            total_price = (quanity * 100) - (quanity * 100 * 0.02) - (quanity * 100 * 0.02 * 0.13)
        if item_name == 'Hammocks':
            total_price = (quanity * 120) - (quanity * 120 * 0.02) - (quanity * 120 * 0.02 * 0.13)
        if item_name == 'Gas Stove':
            total_price = (quanity * 50) - (quanity * 50 * 0.02) - (quanity * 50 * 0.02 * 0.13)
        else:
            total_price = (quanity * 40) - (quanity * 40 * 0.02) - (quanity * 40 * 0.02 * 0.13)
